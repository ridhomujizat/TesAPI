...
export function CollectionsSidebar({ onToast, onViewChange, onWorkspaceChange }: Props) {
  ...
  const { tabs, activeTabId, openRequest, renameSavedTab, closeSavedTabs } = useRequestStore();
  ...
  const confirmDelete = async () => {
    ...
    try {
      const store = useCollectionStore.getState();
      if (deleteTarget.nodeId) closeSavedTabs(deleteTarget.collectionId, await store.deleteNode(deleteTarget.collectionId, deleteTarget.nodeId));
  ...
  return <>
    ...
    <div className="tree virtual-tree" ref={treeRef} onScroll={(event) => setScrollTop(event.currentTarget.scrollTop)} onContextMenu={(event) => { event.preventDefault(); const fallback = collectionState.summaries.find((summary) => collectionState.expandedIds[summary.id])?.id ?? collectionState.summaries[0]?.id ?? ''; setContext({ x: event.clientX, y: event.clientY, collectionId: fallback, type: 'empty' }); }}>
      ...
      <div className="virtual-tree-space" style={{ height: rows.length * ROW_HEIGHT }}>{rows.slice(start, end).map((row, offset) => { const top = (start + offset) * ROW_HEIGHT; if (row.type === 'collection') { const expanded = !!collectionState.expandedIds[row.collection.id]; return <div className={`virtual-tree-row collection-tree-row${drop?.key === row.key ? ` drop-${drop.mode}` : ''}`} key={row.key} style={{ top }} onDragOver={(event) => dragOver(event, row)} onDragLeave={() => setDrop(null)} onDrop={(event) => { event.preventDefault(); void dropOn(row); }} onContextMenu={(event) => { event.preventDefault(); event.stopPropagation(); setContext({ x: event.clientX, y: event.clientY, collectionId: row.collection.id, type: 'collection' }); }}><div className="collection-row-main" role="button" tabIndex={0} onClick={() => void toggleCollection(row.collection)} onKeyDown={(event) => { if (event.key === 'Enter') void toggleCollection(row.collection); }}>{expanded ? <ChevronDown size={12} /> : <ChevronRight size={12} />}<Folder size={13} />{inlineName(row.collection.id, row.collection.name)}<small>{row.collection.requestCount}</small></div></div>; } const expanded = row.type === 'folder' && !!collectionState.expandedIds[row.node.id]; const selected = row.type === 'request' && activeOrigin?.collectionId === row.collectionId && activeOrigin.nodeId === row.node.id; const isEditing = editing?.collectionId === row.collectionId && editing.nodeId === row.node.id; return <div className={`virtual-tree-row node-tree-row${row.type === 'request' ? ' request' : ''}${selected ? ' selected' : ''}${drop?.key === row.key ? ` drop-${drop.mode}` : ''}`} key={row.key} role="button" tabIndex={0} style={{ top, paddingLeft: 8 + row.depth * 16 }} draggable={!isEditing} onDragStart={(event) => { setDragging({ collectionId: row.collectionId, nodeId: row.node.id }); event.dataTransfer.setData('text/plain', row.node.id); event.dataTransfer.effectAllowed = 'move'; }} onDragEnd={() => { setDragging(null); setDrop(null); }} onDragOver={(event) => dragOver(event, row)} onDragLeave={() => setDrop(null)} onDrop={(event) => { event.preventDefault(); event.stopPropagation(); void dropOn(row); }} onContextMenu={(event) => { event.preventDefault(); event.stopPropagation(); setContext({ x: event.clientX, y: event.clientY, collectionId: row.collectionId, nodeId: row.node.id, type: row.type }); }} onClick={() => openNode(row.collectionId, row.node)} onKeyDown={(event) => { if (event.key === 'Enter' && !isEditing) openNode(row.collectionId, row.node); }}>{row.type === 'folder' ? (expanded ? <ChevronDown size={12} /> : <ChevronRight size={12} />) : <span className="tree-method" style={{ color: methodColor(row.node.type === 'request' ? row.node.request.method : 'GET') }}>{row.node.type === 'request' ? row.node.request.method : ''}</span>}{row.type === 'folder' && <Folder size={13} />}{inlineName(row.collectionId, row.node.name, row.node.id)}</div>; })}</div>
    </div>
